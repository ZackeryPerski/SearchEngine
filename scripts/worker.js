...
function findKeyWordsInHTML(htmlData){
    ...
    if(metaTags){
        tags = metaTags.split(',').map(tag => tag.trim()).filter(tag => tag.length >= 3); //Filter here removes tags that are 2 characters or less.
    ...
    if(titleTags){
        let tempTags = titleTags.split(' ').map(tag => tag.trim()).filter(tag => tag.length >= 3);
    ...
    if(h1Tags){
        let tempTags = h1Tags.split(' ').map(tag => tag.trim()).filter(tag => tag.length >= 3);
        tags = tags.concat(tempTags);
        if (tags.length > keyWordLimit){
            tags = tags.slice(0, keyWordLimit);
            return tags;
    ...
    if(h2Tags){
        let tempTags = h2Tags.split(' ').map(tag => tag.trim()).filter(tag => tag.length >= 3);
        tags = tags.concat(tempTags);
        if (tags.length > keyWordLimit){
            tags = tags.slice(0, keyWordLimit);
            return tags;
    ...
    if(h3Tags){
        let tempTags = h3Tags.split(' ').map(tag => tag.trim()).filter(tag => tag.length >= 3);
        tags = tags.concat(tempTags);
        if (tags.length > keyWordLimit){
            tags = tags.slice(0, keyWordLimit);
            return tags;
    ...
    if(h4Tags){
        let tempTags = h4Tags.split(' ').map(tag => tag.trim()).filter(tag => tag.length >= 3);
        tags = tags.concat(tempTags);
        if (tags.length > keyWordLimit){
            tags = tags.slice(0, keyWordLimit);
            return tags;
    ...
    if(h5Tags){
        let tempTags = h5Tags.split(' ').map(tag => tag.trim()).filter(tag => tag.length >= 3);
        tags = tags.concat(tempTags);
        if (tags.length > keyWordLimit){
            tags = tags.slice(0, keyWordLimit);
            return tags;
    ...
    if(h6Tags){
        let tempTags = h6Tags.split(' ').map(tag => tag.trim()).filter(tag => tag.length >= 3);
        tags = tags.concat(tempTags);
        if (tags.length > keyWordLimit){
            tags = tags.slice(0, keyWordLimit);
            return tags;
    ...
    if(bodyTags){
        let tempTags = bodyTags.split(' ').map(tag => tag.trim()).filter(tag => tag.length >= 3);
        tags = tags.concat(tempTags);
        if (tags.length > keyWordLimit){
            tags = tags.slice(0, keyWordLimit);
            return tags;
...
function main(){
    ...
    .then((htmlData) => {
        ...
        if(htmlData !== null && htmlData !== undefined && typeof htmlData === 'string'){
            //Parse the html data
